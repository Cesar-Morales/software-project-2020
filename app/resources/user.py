...
@login_required
def index(page=1):
    """ Funcion que muestra el listado de usuarios. Verifica si el usuario 
    esta autenticado, si esta, verifica los permisos, si no tiene, lo 
    devuelve al home, si los tiene lo dejo ver el listado. Si no esta 
    autenticado, le mostramos que no tiene autorizacion """
    
    if not check_permission('user_index'):
        flash('No posee los permisos necesarios ' +
              'para poder ver la lista de usuarios')
        return redirect(url_for("home"))
    
    permiso = check_permission('user_show')
    per_page = Site.page()
    total=User.getAll().count()
    users = User.getAll().paginate(page,per_page,False)
...
@login_required
def new():
    """ Controlador que renderiza el template para creacion de usuario. 
...
@login_required
def create():
    """ Controlador que realiza el intento de creacion de usuario. 
    Se verifica que se este autenticado y que se tengan los permisos. 
    Si alguna verificacion no pasa, se redirecciona segun corresponda """
...
@login_required
def block():
    """ Controlador que realiza el intento de bloqueo de usuario. 
    Se verifica que se este autenticado y que se tengan los permisos. 
    Si alguna verificacion no pasa, se redirecciona segun corresponda.
...
@login_required
def activate():
    """ Controlador que realiza el intento de activacion de usuario. 
    Se verifica que se este autenticado y que se tengan los permisos. 
    Si alguna verificacion no pasa, se redirecciona segun corresponda. 
    Si pasa las verificaciones, informa estado de operacion 
    segun corresponda """
...
@login_required
def edit():
    """ Controlador que se encarga de renderizar el template correspondiente 
    a la edicion del usuario seleccionado. Se obtienen a partir del model, 
    los datos en cuestion se muestra el formulario, y las opciones para 
    editar los mismos. Se verifica que se este autenticado y que se tengan 
    los permisos. Si alguna verificacion no pasa, se redirecciona segun 
    corresponda. Si pasa las verificaciones, informa estado de operacion 
    segun corresponda """
    ...
    return render_template("user/editar.html", 
                           form = form, 
                           roles = roles, 
                           rolesUser = rolesUser)

def confirmEdit():
    """Controlador que recibe los datos del template de editar usuario e 
    intenta hacer el update correspondiente. Se verifica que se este 
    autenticado y que se tengan los permisos. Si alguna verificacion no pasa, 
    se redirecciona segun corresponda. Si pasa las verificaciones, informa 
    estado de operacion segun corresponda"""
